...
class WordLadderII_others {
  ...
  constructor(
    ...
    this.wordSet = new Set([beginWord, ...wordList]);
  ...
  private bfs(): void {
    ...
    while (queue.length) {
      ...
      const nxtLvlWords = this.adjList.get(word) || [];

      for (let i = 0; i < nxtLvlWords.length; i++) {
        const newWord = nxtLvlWords[i];
...
const wl = new WordLadderII_others(b, e, wordList);
